...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # replaced code from original spec by adding observationUpdate which updates the belief state based on what is inferred. 
        allPossible = util.Counter()
        for pos in self.legalPositions:
            trueDistance = util.manhattanDistance(pos, pacmanPosition)
            if emissionModel[trueDistance] > 0:
                # print "self.beliefs in for loop"
                # print self.beliefs[pos]
                observationUpdate = self.beliefs[pos] * emissionModel[trueDistance] 
                # print "observationUpdate"
                # print observationUpdate 
                allPossible[pos] = observationUpdate

        #Special case: when a ghost is captured by pacman
        jailPos = self.getJailPosition()
        if noisyDistance == None: 
            # reset allPossible and set jailPos key to 1
            allPossible = util.Counter()
            allPossible[jailPos] = 1.0

        "*** END YOUR CODE HERE ***"
        allPossible.normalize()
        # print "allPossible normalized"
        # print allPossible
        self.beliefs = allPossible
        # print "self.beliefs updated"
        # print self.beliefs
        # print("allPossible now normazlied")
        # print(allPossible)
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for prevPos in self.legalPositions:
            ...
            for newPos in newPosDist:
                previous_iter = self.beliefs[prevPos]
                transition_model = newPosDist[newPos]
                updateBeliefs[newPos] += previous_iter*transition_model
                # print "updateBeliefs[newPos]"
                # print updateBeliefs[newPos]

                # algorithm from slide 14: MarkovModels.pdf
                # markov chain forward alg: sum of all x_{0}(P(x_{0}) * P(x_{0} | x_{0+1})
                # self.beliefs[prevPos] = P(x_{0}) (previous iteration)
                # newPostDist[newPos] = P(x_{0} | x_{0+1} (transition model)
                # newPostDist[newPos] = Pr(ghost is at position newpos at time t + 1 | ghost is at position oldPos at time t ) 
    
        # similar to observe method, must normalize and update belief states
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        density = self.numParticles / len(self.legalPositions)
        counter = 0
        for x,y in self.legalPositions:
            for i in range(density):
                self.particles.append((x,y))
            #print("particles len", len(self.particles))
            counter += density
        if counter < self.numParticles:
            #print("uh oh, counter less than num particles. do things!")
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        # Special Case: ghost gets eaten by pacman
        jailPos = self.getJailPosition()
        if noisyDistance == None:
            ...
            #print("len beliefs", len(oldBeliefs))
            #print("len legal posistions", len(self.legalPositions))
            for p in oldBeliefs:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    #print("position", p)
                    weights[p] += emissionModel[trueDistance]*oldBeliefs[p]

            weights.normalize() # normalize weights to resample; handled already in util.sample

            # Special Case: all particle weights are zero
            if weights.totalCount() == 0:
                ...
                for i in range(self.numParticles):
                    # weight[position] * self.numParticles => number of particles at that location
                    self.particles.append(util.sample(weights))

                """
                # alternate to above, multiply weights by num particles
                self.particles = list()
                for key in weights:
                    self.particles += [key] * (int(round(self.numParticles * weights[key])))
    ...
    def elapseTime(self, gameState):
        ...
        for prevPos in self.particles:
            if prevPos not in memo.keys():
                memo[prevPos] = self.getPositionDistribution(self.setGhostPosition(gameState, prevPos))
            #newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, prevPos))
            newPosDist = memo[prevPos]
            tmp.append(util.sample(newPosDist))
            #print("newPostDist", newPosDist)
        self.particles = tmp
        """
        # alternate, faster way than above; but not working T_T
        for prevPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, prevPos))
            self.particles.append(util.sample(newPosDist))
            #print("newPostDist", newPosDist)
            self.particles += [prevPos] * (int(round(self.numParticles * newPosDist[prevPos])))
        """
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.particles:
            #loc_count = self.particles.count(p) # efficent?
            beliefs[p] += 1
        beliefs.normalize()
        return beliefs
...
class JointParticleFilter:
    ...
    def __init__(self, numParticles=600):
        self.setNumParticles(numParticles)
        self.particles = list() # element e = ((x,y), (a,b))
        self.positions = list()
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        r = self.numGhosts
        # positions ((9, 9), (8, 9)) as tuples of tuple positions a ghost may occupy
        positions = list(itertools.product(self.legalPositions, repeat = r))
        # to create randomly ordered permutations of ghost positions
        random.shuffle(positions)
        self.positions = positions
        #print("positions", positions)
        #print("len positions", len(positions))

        density = self.numParticles / len(positions)
        counter = 0
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        weights = util.Counter() # outside loop to calculate weights based on ALL ghosts
        #oldBeliefs = self.getBeliefDistribution()
        for p in self.particles:
            weight = 1.0
            #jParticle = None
            #noisy = False
            for i in range(self.numGhosts):
                # Special Case: ghost has been eaten by pacman
                if noisyDistances[i] == None:
                    jParticle = self.getParticleWithGhostInJail(p, i)
                    #noisy = True
                    p = jParticle
                else:
                    trueDistance = util.manhattanDistance(p[i], pacmanPosition)
                    # because joint distribution
                    #weight *= emissionModels[i][trueDistance]*oldBeliefs[p]
        ...
        # Special Case: weights are zero so reinitialize
        if weights.totalCount() == 0:
            print("reinitializing")
            self.particles = list()
            self.initializeParticles()
            # remove jailed ghosts from reinitialized states
            for i in xrange(self.numGhosts):
                if noisyDistances[i] == None:
                    for j in xrange(len(self.particles)):
                        jParticle = self.getParticleWithGhostInJail(j, i)
                        self.particles[j] = jParticle
        else:
            # update particle composition based on weights
            self.particles = list()
            for i in range(self.numParticles):
            ...
            """
            # alternative to above, multiply weights by num particles
            self.particles = list()
            for key in weights:
                #print key
                self.particles += [key] * (int(round(self.numParticles * weights[key])))
                #print("len particles updated: ", len(self.particles))
            """
        #newBeliefes = self.getBeliefDistribution()
        #print("newBeliefes", newBeliefes)
